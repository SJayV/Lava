...
import { N_LOCAL, RADIUS_TO_H_RATIO, computeCalibration, initializeDripSimulation, stepDripSimulation } from './src/simulation.js';
...
async function initializeWorld(canvas, device) {
  ...
  const { radius, isoLevel, gradientMagnitudeMax } = await computeCalibration(device, { smoothingRadius: h, radiusRatio: RADIUS_TO_H_RATIO, fluidDensity: FLUID_DENSITY, localNeighborCount: N_LOCAL });
  const traceBounds = computeTraceBounds({ lineSpanWidth, h });

  return { aspectRatio, lineSpanWidth, cameraBasis, focalLength, h, radius, isoLevel, gradientMagnitudeMax, traceBounds };
}
...
async function initialize() {
  ...
  const { anchorBuffer, dropState, pairState } = initializeSceneState(graphicsContext.device, registry, { pairCount: PAIR_COUNT, lineSpanWidth: world.lineSpanWidth, lineY: LINE_Y, radius: world.radius });
  const dripSimulation = initializeDripSimulation(graphicsContext.device, anchorBuffer, dropState, pairState, { h: world.h, fluidDensity: FLUID_DENSITY, respawnY: RESPAWN_Y, baseRadius: world.radius, pairCount: PAIR_COUNT });
